...
(function ($) {
    ...
    */
    ...
    var SBUsers = {
        ...
        search: function (search) {
            let search_icon = $(users_area).find('.sb-search-btn i');
            if ($(search_icon).sbLoading()) return;
            $(search_icon).sbLoading(true);
            SBF.search(search, () => {
                this.loading();
                users_pagination = 1;
                SBF.ajax({
    ...
    */
    ...
    var SBConversations = {
        ...
        openConversation: function (conversation_id, user_id, scroll = true) {
        ...
        search: function (search) {
            let search_icon = $(conversations_admin_list).find('.sb-search-btn i');
            if ($(search_icon).sbLoading()) return;
            $(search_icon).sbLoading(true);
            SBF.search(search, () => {
                if (search.length > 1) {
    ...
    */
    ...
    var SBProfile = {
        ...
        profileRow: function (key, value, name = key) {
            if (value == '') return '';
            let icons = { 'id': 'padlock', 'full_name': 'user', 'email': 'envelope', 'user_type': 'user', 'last_activity': 'reload', 'creation_time': 'calendar', 'token': 'shuffle', 'currency': 'currency', 'browser': 'smartphone', 'ip': 'internet', 'location': 'marker', 'country': 'marker', 'city': 'marker', 'os': 'desktop', 'current_url': 'next', 'timezone': 'clock' };
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets' };
                let updates = false;
        ...
        */
        ...
        SBF.ajax({
            ...
            if (SBF.getURL('conversation') && SBF.getURL('user_id')) {
                SBConversations.openConversation(SBF.getURL('conversation'), SBF.getURL('user_id'));
            } else if (!responsive) {
                SBConversations.clickFirst();
            }
            SBConversations.startRealTime();
        ...
        $(conversations_admin_list).find('.sb-scroll-area').on('scroll', function () {
            if (!is_busy && !SBConversations.is_search && $(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
        ...
        $(conversations_admin_list).find('.sb-search-btn input').on('input', function () {
        ...
        */
        ...
        $(users_table).parent().on('scroll', function () {
            if (!is_busy && SBUsers.search_query == '' && ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight)) {
                is_busy = true;
        ...
        $(settings_area).on('click', '#tab-slack', function () {
            ...
            SBF.ajax({
                ...
                if (SBF.errorValidation(response, 'slack-token-not-found')) {
                    code = `<p>${sb_('Synchronize Slack and save changes before linking any agents.')}</p>`;
                } else {
        ...
        */
        ...
        $(admin).on('click', '.sb-nav li', function () {
            ...
            $(area).find(' > .sb-content > div').sbActivate(false).eq($(this).index()).sbActivate();
