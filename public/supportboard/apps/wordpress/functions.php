...
function sb_wp_get_active_user($user_id) {
    $user = sb_wp_get_user($user_id);
    $query = '';
    if (isset($user['email'])) {
        $query = 'SELECT id, token FROM sb_users WHERE email ="' . $user['email'] . '" LIMIT 1';
        $user_db = sb_db_get($query);
        if ($user_db === '') {
            $settings_extra = ['wp-id' => [$user['id'], 'WordPress ID']];
            ...
            if ($active_user && ($active_user['user_type'] == 'lead' || $active_user['user_type'] == 'visitor')) {
                ...
                sb_add_user($user, $settings_extra, false, false);
            }
...
function sb_wp_synch() {
    ...
    for ($i = 0; $i < count($users); $i++) {
        if (!in_array($users[$i]['user_email'], $emails_array)) {
            ...
            $settings_extra = [];
            if (defined('SB_UMP')) {
                $settings_extra = sb_ump_get_user_extra($users[$i]['ID']);
                $settings['profile_image'] = sb_ump_get_profile_image($users[$i]['ID'], 'user');
            }
