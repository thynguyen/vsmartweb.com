...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets'];
...
function sb_update_login($profile_image, $first_name, $last_name, $email, $department = '', $user_type = false) {
    ...
    if (isset($_SESSION['sb-session'])) {
        ...
        $settings['department'] = $department == 'NULL' || $department == '' ? null : $department;
        if ($user_type != false) {
...
function sb_get_active_user($storage = null, $db = false, $login_app = false) {
    ...
    if ($return === false && $storage != false && isset($storage)) {
...
function sb_js_admin() {
    $code = '<script>';
    $code .= 'var SB_ADMIN_SETTINGS = { "bot-id": ' . sb_get_bot_id() . ', "close-message": ' . (sb_get_multi_setting('close-message', 'close-active') ? 'true' : 'false') . ', "routing": ' . ((!isset($_SESSION['sb-session']) || $_SESSION['sb-session']['user_type'] == 'agent') && (sb_get_multi_setting('queue', 'queue-active') || sb_get_setting('routing')) ? 'true' : 'false') . ', "desktop-notifications": "' . sb_get_setting('desktop-notifications') .'", "push-notifications": "' . sb_get_multi_setting('push-notifications', 'push-notifications-active') . '", "flash-notifications": "' . sb_get_setting('flash-notifications') . '", "notifications-icon" : "' . (empty(sb_get_setting('notifications-icon')) ? SB_URL . '/media/icon.png': '') . '", "auto-updates": "' . sb_get_setting('auto-updates') .'", "sounds": "' . sb_get_setting('chat-sound-admin') . '" };';
    if (isset($_SESSION['sb-session'])) {
        $session = $_SESSION['sb-session'];
        $code .= 'var SB_ACTIVE_AGENT = { id: "' . $session['id'] . '", full_name: "' . $session['first_name'] . ' ' .  $session['last_name'] . '", user_type: "' . $session['user_type'] . '", profile_image: "' . $session['profile_image'] . '", department: "' . (isset($session['department']) ? $session['department'] : '') . '"};';
    } else {
    ...
    $code .= 'var SB_VERSIONS = { sb: "' . SB_VERSION . '", dialogflow: "' . (defined('SB_DIALOGFLOW') ? SB_DIALOGFLOW : -1) . '", slack: "' . (defined('SB_SLACK') ? SB_SLACK : -1) . '", tickets: "' . (defined('SB_TICKETS') ? SB_TICKETS : -1) . '"};';
    $code .= '</script>';
    echo $code;
}
...
function sb_add_user($settings = [], $settings_extra = [], $login_app = false) {
    ...
    $query = 'INSERT INTO sb_users(first_name, last_name, password, email, profile_image, user_type, creation_time, token, department, last_activity) VALUES ("' . sb_db_escape(htmlentities($settings['first_name'], ENT_NOQUOTES)) . '", "' . sb_db_escape(htmlentities($settings['last_name'], ENT_NOQUOTES)) . '", "' . sb_db_escape(htmlentities($settings['password'], ENT_NOQUOTES)) . '", ' . ($settings['email'] == '' ? 'NULL' : '"' . sb_db_escape(htmlentities($settings['email'], ENT_NOQUOTES)) . '"') . ', "' . sb_db_escape(htmlentities($settings['profile_image'], ENT_NOQUOTES)) . '", "' . $settings['user_type'] . '", "' . $now . '", "' . $token . '", ' . $settings['department'] . ', "' . $now . '")';
    ...
    if (!sb_is_error($result) && $settings['user_type'] == 'user' && !sb_is_agent()) {
...
function sb_add_new_user_extra($user_id, $settings) {
    ...
    foreach ($settings as $key => $setting) {
        if (is_array($setting) && $setting[0] != '') {
            $query .= '("' . $user_id . '", "' . $key . '", "' . sb_db_escape(htmlentities($setting[1], ENT_NOQUOTES)) . '", "' . sb_db_escape(htmlentities($setting[0], ENT_NOQUOTES)) . '"),';
        }
    }
    if ($query != '') {
...
function sb_update_user($user_id, $settings, $settings_extra = []) {
    ...
    $department = sb_isset($settings, 'department', ['NULL'])[0];
...
function sb_get_users($sorting = ['creation_time' , 'DESC'], $user_types = [], $search = '', $pagination = 0) {
    ...
    $users = sb_db_get(SELECT_FROM_USERS . ' FROM sb_users ' . $query . ' ORDER BY ' . $sorting[0] . ' ' . $sorting[1] . ' LIMIT ' . ($pagination * 100 + $index) . ',100', false);
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $routing = false) {
    ...
    $conversation_id = sb_db_query('INSERT INTO sb_conversations(user_id, title, status_code, creation_time, department, agent_id) VALUES ("' . $user_id . '", "' . sb_db_escape(htmlentities(ucfirst($title), ENT_NOQUOTES)) . '", "' . $status_code . '", "' . gmdate('Y-m-d H:i:s') . '", ' . (sb_isset_num($department) ? $department : 'NULL') . ', ' . (sb_isset_num($agent_id) ? $agent_id : 'NULL') . ')', true);
...
function sb_send_message($user_id = -1, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false) {
    ...
    if ($user_id != -1) {
        ...
        if ($security) {
            ...
            $response = sb_db_query('INSERT INTO sb_messages(user_id, message, status_code, creation_time, attachments, conversation_id, payload) VALUES ("' . $user_id . '", "' . sb_db_escape(htmlentities($message, ENT_NOQUOTES)) . '", 0, "' . gmdate('Y-m-d H:i:s') . '", "' . $attachments_json . '", "' . $conversation_id . '", "' . ($payload != false ? sb_db_json_enconde($payload) : '') . '")', true);
...
function sb_init_translations() {
    ...
    if (isset($SB_LANGUAGE) && $SB_LANGUAGE != false && $SB_LANGUAGE[0] !='en') {
...
function sb_get_user_language() {
    return isset($_GET['lang']) ? $_GET['lang'] : strtolower(substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2));
}
...
function sb_get_multi_setting($id, $sub_id) {
    $setting = sb_get_setting($id);
    if ($setting != false && (!isset($setting[$id . '-active']) || $setting[$id . '-active'] == true) && isset($setting[$sub_id])) {
...
function sb_get_front_settings() {
    ...
    $return = [
        ...
        'registration-timetable' => sb_isset($settings, 'registration-timetable', [false])[0],
        'registration-offline' => sb_isset($settings, 'registration-offline', [false])[0],
        'registration-link' => sb_isset($settings, 'registration-link', [''])[0],
        ...
        'popup-mobile-hidden' => isset($settings['popup-message']) && isset($settings['popup-message'][0]['popup-mobile-hidden']) ? $settings['popup-message'][0]['popup-mobile-hidden'][0] : false,
        'welcome' => sb_isset($settings, 'welcome-message', [['welcome-active' => [false]]])[0]['welcome-active'][0],
        'welcome-trigger' => isset($settings['welcome-message']) && isset($settings['welcome-message'][0]['welcome-trigger']) ? $settings['welcome-message'][0]['welcome-trigger'][0] : 'load',
        'welcome-delay' => isset($settings['welcome-message']) && isset($settings['welcome-message'][0]['welcome-delay']) ? intval($settings['welcome-message'][0]['welcome-delay'][0]) : 2000,
        'follow' => sb_isset($settings, 'follow-message', [['follow-active' => [false]]])[0]['follow-active'][0],
        'follow-delay' => isset($settings['follow-message']) && isset($settings['follow-message'][0]['follow-delay']) ? $settings['follow-message'][0]['follow-delay'][0] : '',
        'chat-manual-init' => sb_isset($settings, 'chat-manual-init', [false])[0],
        'chat-login-init' => sb_isset($settings, 'chat-login-init', [false])[0],
        'chat-sound' => sb_isset($settings, 'chat-sound', ['n'])[0],
        ...
        'push-notifications' => sb_isset($settings, 'push-notifications', [['push-notifications-active' => [false]]])[0]['push-notifications-active'][0],
        'notifications-icon' => sb_isset($settings, 'notifications-icon', [SB_URL . '/media/icon.png'])[0],
        'bot-id' => sb_get_bot_id(),
        'bot-name' => sb_isset($settings, 'bot-name', [''])[0],
        'bot-image' => sb_isset($settings, 'bot-image', [''])[0],
        'bot-delay' => intval(sb_isset($settings, 'dialogflow-bot-delay', [2000])[0]),
        'bot-office-hours' => sb_isset($settings, 'dialogflow-timetable', [false])[0],
        'bot-unknow-notify' => sb_isset($settings, 'bot-unknow-notify', [false])[0],
        ...
        'disable-offline' => sb_isset($settings, 'chat-offline-disable', [false])[0],
        'timetable' => sb_isset($settings, 'chat-timetable', [['chat-timetable-active' => [false]]])[0]['chat-timetable-active'][0],
        'timetable-hide' => isset($settings['chat-timetable']) && isset($settings['chat-timetable'][0]['chat-timetable-hide']) ? $settings['chat-timetable'][0]['chat-timetable-hide'] : false,
        'articles' => sb_isset($settings, 'articles-active', [false])[0],
        'articles-title' => sb_isset($settings, 'articles-title', [''])[0],
        'init-dashboard' => sb_isset($settings, 'init-dashboard', [false])[0] && !sb_isset($settings, 'disable-dashboard', [false])[0],
        'wp' => defined('SB_WP'),
        'wp-users-system' => sb_isset($settings, 'wp-users-system', ['sb'])[0],
        'queue' => sb_isset($settings, 'queue', [['queue-active' => [false]]])[0]['queue-active'][0],
        'queue-message' => sb_isset($settings, 'queue', [['queue-message' => ['']]])[0]['queue-message'][0],
        'queue-message-success' => sb_isset($settings, 'queue', [['queue-message-success' => ['']]])[0]['queue-message-success'][0],
        'queue-response-time' => sb_isset($settings, 'queue', [['queue-response-time' => [5]]])[0]['queue-response-time'][0],
        'routing' => !sb_isset($settings, 'queue', [['queue-active' => [false]]])[0]['queue-active'][0] && sb_isset($settings, 'routing', [false])[0],
        'webhooks' => sb_isset($settings, 'webhooks', [['webhooks-active' => [false]]])[0]['webhooks-active'][0],
        'agents-online' => intval(sb_db_get('SELECT COUNT(*) as count FROM sb_users WHERE (user_type = "agent" OR user_type = "admin") AND last_activity > "' . gmdate('Y-m-d H:i:s', time() - 15) . '"')['count']) > 0
    ];
    if ($return['timetable-hide']) {
        $return['timetable-message'] = '*' . $settings['chat-timetable'][0]['chat-timetable-title'][0] . '*\n' . $settings['chat-timetable'][0]['chat-timetable-msg'][0];
    }
    if (defined('SB_TICKETS')) {
        $return['tickets-registration-required'] = sb_isset($settings, 'tickets-registration-required', [false])[0];
        $return['tickets-registration-redirect'] = sb_isset($settings, 'tickets-registration-redirect', [''])[0];
        $return['tickets-default-form'] = sb_isset($settings, 'tickets-default-form', ['login'])[0];
    }
...
function sb_get_block_setting($value) {
    switch ($value)  {
    	case 'privacy':
            ...
            return ['title' => $setting['privacy-title'], 'message' => $setting['privacy-msg'], 'decline' => $setting['privacy-msg-decline'], 'link' => $setting['privacy-link'], 'link-name' => sb_isset($setting, 'privacy-link-text', ''), 'btn-approve' => $setting['privacy-btn-approve'], 'btn-decline' => $setting['privacy-btn-decline']];
        case 'popup':
...
function sb_get_departments() {
    ...
    if ($count) {
        for ($i = 0; $i < $count; $i++) {
            $departments[$items[$i]['department-id']] = ['name' => $items[$i]['department-name'], 'color' => $items[$i]['department-color'], 'image' => (isset($items[$i]['department-image']) ? $items[$i]['department-image'] : '')];
        }
    }
    return $departments;
}
...
function sb_curl($url, $post_fields = '', $header = [], $type = 'POST') {
    ...
    curl_close($ch);
    return $type == 'POST' ? json_decode($response, true) : $response;
}
...
/*
 * -----------------------------------------------------------
 * # GET RICH MESSAGES IDS
 * -----------------------------------------------------------
 *
 * Get the custom rich messages ids including the built in ones
 *
 */
 ...
 * -----------------------------------------------------------
 *
 * Get the rich message with the given name
 *
 */

function sb_get_rich_message($name) {
    if (in_array($name, ['email', 'registration', 'registration-tickets', 'login', 'login-tickets', 'timetable', 'articles'])) {
        $title = '';
        $message = '';
        $success = '';
        switch ($name) {
            case 'email' :
                ...
                $placeholder = sb_(sb_isset($settings, 'follow-placeholder', 'Email'));
                $success = sb_(sb_isset($settings, 'follow-success'));
            ...
            case 'articles':
                ...
                $code = '<div class="sb-dashboard-articles"><div class="sb-title">' . sb_($articles_title == '' ? 'Help Center' : $articles_title) . '</div><div class="sb-input sb-input-btn"><input placeholder="' . sb_('Search for articles...') . '" autocomplete="off"><div class="sb-submit sb-icon-arrow-right"></div></div><div class="sb-articles">';
...
function sb_set_rating($settings, $payload = false, $message_id = false, $message = false, $user_id = false) {
    ...
    if (isset($settings['rating'])) {
        ...
        if ($message_id != false && $user_id != false) {
            sb_update_message($user_id, $message_id, $message, false, $payload);
        }
        return true;
    }
...
function sb_get_articles($id = -1, $count = false, $full = false) {
    ...
    if ($articles != false) {
        ...
        if ($id == -1) {
            ...
            for ($i = 0; $i < $count; $i++) {
                if ($articles[$i]['title'] != '') {
                    if (!$full && strlen($articles[$i]['content']) > 100) {
...
function sb_css() {
    ...
    if ($color_1 != '') {
        $css .= '.sb-chat-btn, .sb-chat>div>.sb-header,.sb-chat .sb-dashboard>div>.sb-btn:hover,.sb-chat .sb-scroll-area .sb-header,.sb-input.sb-input-btn>div,div ul.sb-menu li:hover,
                 .sb-select ul li:hover,.sb-popup.sb-emoji .sb-emoji-bar>div.sb-active, .sb-popup.sb-emoji .sb-emoji-bar>div:hover,.sb-btn,.sb-rich-message[disabled] .sb-buttons .sb-btn { background-color: ' . $color_1 . '; }';
        ...
        $css .= '.sb-chat .sb-dashboard>div>.sb-btn,.sb-editor .sb-bar-icons>div:hover:before,.sb-articles>div:hover>div,.sb-main .sb-btn-text:hover,.sb-editor .sb-submit,
                 ...
                 .sb-input.sb-input-image .image:hover:before,.sb-rich-message .sb-card .sb-card-btn,.sb-rich-message.sb-rich-slider .sb-arrow:hover,.sb-loading:before { color: ' . $color_1 . '; }';
    ...
    if ($color_2 != '') {
        $css .= '.sb-chat-btn:hover,.sb-input.sb-input-btn>div:hover,.sb-btn:hover,.sb-rich-message .sb-card .sb-card-btn:hover { background-color: ' . $color_2 . '; }';
...
function sb_email_create($recipient_id, $sender_name, $sender_profile_image, $message, $attachments = [], $department = -1) {
    $recipient = false;
    $recipient_name = '';
    $recipient_email = '';
    ...
    if ($recipient_id == 'email-test') {
        ...
        $agents = sb_db_get('SELECT first_name, last_name, email FROM sb_users WHERE (user_type = "agent" OR user_type = "admin")' . ($department == -1 ? '' : ' AND department = ' . $department), false);
        for ($i = 0; $i < count($agents); $i++) {
...
function sb_email($recipient_id, $message, $attachments = [], $sender_id = -1) {
    ...
    if ($sender != false && isset($sender['id']) && $user != false && isset($user['id'])) {
        ...
        $email = sb_email_create_content($emails['email-' . $email_type . '-subject'][0], $emails['email-' . $email_type . '-content'][0], $attachments, ['message' => $message, 'recipient_name' => $user['first_name'] . ' ' . $user['last_name'], 'sender_name' => $sender['first_name'] . ' ' . $sender['last_name'], 'sender_profile_image' => $sender['profile_image']]);
    ...
    if ($zip != false) {
        ...
        if (file_exists($file_path)) {
            ...
            if ($zip->open($file_path) === true) {
                $zip->extractTo($app_name == 'sb' ? str_replace('supportboard', '', SB_PATH) : SB_PATH . '/apps');
                $zip->close();
                unlink($file_path);
                if ($app_name == 'sb') {
...
function sb_update() {
    ...
    $installed_apps_versions = ['dialogflow' => defined('SB_DIALOGFLOW') ? SB_DIALOGFLOW : -1, 'slack' => defined('SB_SLACK') ? SB_SLACK : -1];
    $keys = sb_get_external_settings('app-keys');
    $result = [];
...
function sb_webhooks($function_name, $parameters) {
    if (isset($function_name) && $function_name != '' && isset($parameters)) {
        ...
        if (isset($names[$function_name])) {
            ...
            if ($webhooks['webhooks-url'] != '' && $webhooks['webhooks-active']) {
                $query = json_encode(['function' => $names[$function_name], 'key' =>$webhooks['webhooks-key'], 'data' => $parameters]);
                if ($query != false) {
...
function sb_system_requirements() {
    ...
    // File permissions
    for ($i = 0; $i < 2; $i++) {
        $path = ($i == 0 ? sb_upload_path() : SB_PATH . '/apps' ) . '/sb-permissions-check.txt';
        sb_file($path, 'permissions-check');
        $checks[($i == 0 ? 'uploads' : 'apps') . '-folder'] = file_exists($path) && file_get_contents($path) == 'permissions-check';
        if (file_exists($path)) {
            unlink($path);
        }
    }
    ...
    $checks['sessions'] = isset($_SESSION['sb-session']);
...
function sb_push_notification($title = '', $message = '', $icon = '', $interest = false, $conversation_id = false) {
    if (sb_get_multi_setting('push-notifications', 'push-notifications-active')) {
        ...
        if (is_numeric($interest) || is_array($interest)) {
            ...
            if (is_numeric($interest) && !sb_is_agent($user_type) && $user['id'] != $interest) return $error;
            if (is_array($interest)) {
                for ($i = 0; $i < count($interest); $i++){
                    if (!sb_is_agent($user_type) && $user['id'] != $interest[$i]) return $error;
                }
            }
        }
        if ($icon == '') {
 ...
 * ----------------------------------------------------------
 *
 * Integration with external services (the below functions are in beta and are not yet available officially)
 *
 */



function sb_subscribe($email, $name = '', $list = '') {
