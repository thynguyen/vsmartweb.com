...
try {
    ...
    if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
        response(transfm('forbidden') . AddErrorLocation(), 403)->send();
        exit;
    }
    ...
    $messages = null;
    if (transfm("Upload_error_messages") !== "Upload_error_messages") {
        $messages = transfm("Upload_error_messages");
    }
    ...
    if (!checkresultingsize($_FILES['files']['size'][0])) {
        ...
        $upload_handler->response['files'][0]->error = sprintf(transfm('max_size_reached'), $config['MaxSizeTotal']) . AddErrorLocation();
