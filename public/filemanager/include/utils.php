...
if ( ! function_exists('transfm'))
{

	/**
	* transfmlate language variable
	*
	* @param $var string name
	*
	* @return string transfmlated variable
	*/
	function transfm($var)
	{
		global $lang_vars;
...
function makeSize($size)
{
	...
	return (number_format($size, 0) . " " . transfm($units[ $u ]));
}
...
function fix_filename($str, $config, $is_folder = false)
{
	...
	if ($config['transfmliteration'])
	{
		if (!mb_detect_encoding($str, 'UTF-8', true))
		{
			$str = utf8_encode($str);
		}
		if (function_exists('transfmliterator_transfmliterate'))
		{
			$str = transfmliterator_transfmliterate('Any-Latin; Latin-ASCII', $str);
		}
		else
		{
			// $str = iconv('UTF-8', 'ASCII//transfmLIT//IGNORE', $str);
			$str = getslugtext($str);
		}
	...
	// Empty or incorrectly transfmliterated filename.
...
function getslugtext($text){
	$slugtext = strtolower(utf8convert($text));
...
function get_file_by_url($url)
{
	...
	curl_setopt($ch, CURLOPT_RETURNtransfmFER, 1);
