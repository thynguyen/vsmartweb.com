...
#                crop 'auto' option.
#              * Added settransfmparency() set method - allows you
#                to turn transfmparency off (like when saving
#                as a jpg).
#              * Added setFillColor() set method - set the
#                background color to use instead of transfmparency.
...
#  Tips:
# -------------------------------
#  * If you're resizing a transfmparent png and saving it as a jpg, set
#  $keeptransfmparency to false with: $magicianObj->settransfmparency(false);
#
#  FEATURES:
#    * EASY TO USE
...
#    * RESIZE IMAGES
#      - Preserve transfmparency (png, gif)
...
class imageLib {
	...
	private $transfmparentArray = array( '.png', '.gif' );
	private $keeptransfmparency = true;
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$this->keeptransfmparancy($optimalWidth, $optimalHeight, $this->imageResized);
...
## --------------------------------------------------------
	...
	private function keeptransfmparancy($width, $height, $im)
		# Author:     Jarrod Oberto
		# Date:       08-04-11
		# Purpose:    Keep transfmparency for png and gif image
		# Param in:
		# Param out:  n/a
		# Reference:
		# Notes:
		#
	{
		// *** If PNG, perform some transfmparency retention actions (gif untested)
		if (in_array($this->fileExtension, $this->transfmparentArray) && $this->keeptransfmparency)
		{
			imagealphablending($im, false);
			imagesavealpha($im, true);
			$transfmparent = imagecolorallocatealpha($im, 255, 255, 255, 127);
			imagefilledrectangle($im, 0, 0, $width, $height, $transfmparent);
		}
		else
		{
...
## --------------------------------------------------------
		...
		#
	{
		...
		$this->keeptransfmparancy($optimalWidth, $optimalHeight, $crop);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$transfmparent = imagecolorallocatealpha($filter, 255, 255, 255, 127);
		imagefilledrectangle($filter, 0, 0, $width, $height, $transfmparent);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addReflection($reflectionHeight = 50, $startingtransfmparency = 30, $inside = false, $bgColor = '#fff', $stretch = false, $divider = 0)
	{
		...
		$transfmaprencyAmount = $this->inverttransfmparency($startingtransfmparency, 100);


		// *** Fade
		if ($stretch)
		{
			$step = 100 / ($reflectionHeight + $startingtransfmparency);
		}
		else
		{
		...
		for ($i = 0; $i <= $reflectionHeight; $i++)
		{
			...
			if ($startingtransfmparency > 100)
			{
				$startingtransfmparency = 100;
			}
			if ($startingtransfmparency < 1)
			{
				$startingtransfmparency = 1;
			}
			imagecopymerge($bg, $li, 0, $i, 0, 0, $this->width, 1, $startingtransfmparency);
			$startingtransfmparency += $step;
		}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function rotate($value = 90, $bgColor = 'transfmparent')
		# Author:     Jarrod Oberto
		# Date:       07-05-2011
		...
		#
	{
		if ($this->imageResized)
		{
			...
			// *** Rotate
			$this->imageResized = imagerotate($this->imageResized, $degrees, $bg); // Rotate 45 degrees and allocated the transfmparent colour as the one to make transfmparent (obviously)

			// Ensure alpha transfmparency
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function roundCorners($radius = 5, $bgColor = 'transfmparent')
		# Author:     Jarrod Oberto
		# Date:       19-05-2011
		...
		#
	{
		...
		// *** Check if the user wants transfmparency
		$istransfmparent = false;
		if ( ! is_array($bgColor))
		{
			if (fix_strtolower($bgColor) == 'transfmparent')
			{
				$istransfmparent = true;
			}
		}


		// *** If we use transfmparency, we need to color our curved mask with a unique color
		if ($istransfmparent)
		{
		...
		// *** Replace the mask color (black) to transfmparent
		imagecolortransfmparent($cornerImg, $maskColor);
		...
		// *** If corners are to be transfmparent, we fill our chromakey color as transfmparent.
		if ($istransfmparent)
		{
			//imagecolortransfmparent($this->imageResized, $imagebgColor);
			$this->imageResized = $this->transfmparentImage($this->imageResized);
			imagesavealpha($this->imageResized, true);
		}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addShadow($shadowAngle = 45, $blur = 15, $bgColor = 'transfmparent')
		#
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addCaptionBox($side = 'b', $thickness = 50, $padding = 0, $bgColor = '#000', $transfmaprencyAmount = 30)
		#
		# Author:   Jarrod Oberto
		# Date:   26 May 2011
		...
		#       (int) $transfmaprencyAmount: The amount of transfmparency to be
		#       applied.
		# Params out: n/a
		# Notes:
		#
	{
		...
		$transfmaprencyAmount = $this->inverttransfmparency($transfmaprencyAmount, 127, false);
		$transfmparent = imagecolorallocatealpha($this->imageResized, $r, $g, $b, $transfmaprencyAmount);
		imagefilledrectangle($this->imageResized, $positionArray['x1'], $positionArray['y1'], $positionArray['x2'], $positionArray['y2'], $transfmparent);
	}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		if (fix_strtolower(strrchr($watermarkImage, '.')) == '.png')
		{
			...
			$opacity = $this->inverttransfmparency($opacity, 100);
...
## --------------------------------------------------------
	...
	public function settransfmparency($bool)
		# Sep 2011
	{
		$this->keeptransfmparency = $bool;
	}
...
## --------------------------------------------------------
	...
	public function setFillColor($value)
		...
		#
		# If the keeptransfmparency is set to false, then no transfmparency is to be used.
...
## --------------------------------------------------------
		...
		#
	{
		...
		{
			if (fix_strtolower($value) == 'transfmparent')
			{
	...
	private function inverttransfmparency($value, $originalMax, $invert = true)
		# Purpose:  This does two things:
		#       1) Convert the range from 0-127 to 0-100
		#       2) Inverts value to 100 is not transfmparent while 0 is fully
		#          transfmparent (like Photoshop)
	{
		// *** Test max range
	...
	private function transfmparentImage($src)
	{
		// *** making images with white bg transfmparent
		$r1 = 0;
		$g1 = 255;
		$b1 = 0;
		for ($x = 0; $x < imagesx($src); ++$x)
		{
			for ($y = 0; $y < imagesy($src); ++$y)
			{
				...
				for ($i = 0; $i < 270; $i++)
				{
					...
					{
						//if ($g == 255) {
						$transfm_colour = imagecolorallocatealpha($src, 0, 0, 0, 127);
						imagefill($src, $x, $y, $transfm_colour);
					}
				}
			}
		}
