	...
	<div id="dnsconfig">
		<div class="card">
			<div class="card-body">
				...
				<button type="button" class="btn btn-sm btn-primary" @click="saveConfig" v-if="(cloudFlare.email && cloudFlare.apikey) && cloudFlare.apikey !== hideapikey">{{trans('Langcore::global.Save')}}</button>
				<button type="button" class="btn btn-sm btn-warning" @click="showApi" v-if="cloudFlare.apikey!==hideapikey">{{trans('Langcore::global.Cancel')}}</button>
			</div>
		</div>
	</div>
</template>
