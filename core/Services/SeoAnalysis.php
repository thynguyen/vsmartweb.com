        ...
        if ($images->length > 0) {
            ...
            foreach ($imgs as $mg) {
                ...
                if (isset($mg['src']) && !empty($mg['src'])) {
                    ...
                    $class = (!empty($mg['class']))?'class&equals;&quot;'.$mg['class'].'&quot;':'';
                    $alt = (!empty($mg['alt']))?'alt&equals;&quot;'.$mg['alt'].'&quot;':'';
                    $mg['tabimg'] = '&lt;img src&equals;&quot;'.$mg['src'].'&quot; '.$class.' '.$alt.'&sol;&gt;';
                    if (empty($mg['alt'])) {
                        $mg['imgemptyalt'] = '&lt;img src&equals;&quot;'.$mg['src'].'&quot; '.$class.'&sol;&gt;';
                    }
                    if (!empty($info)) {
                        ...
                        if ($size) {
                            $sizeKb = round(SeoHelper::fileSize($mg['src']) / 1000);
        ...
        foreach ($files as $file) {
            foreach ($file->attributes as $attr) {
                if ($attr->name == $attrName) {
                    $sizeKb = !empty($size) ? round(SeoHelper::fileSize($this->linkBuilder($attr->nodeValue)) / 1000) : null;
                    if ($size) {
                        $this->resourceSize += $sizeKb;
                    }
                    $retFiles[] = [
