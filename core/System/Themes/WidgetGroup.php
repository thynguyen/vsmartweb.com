...
use Vsw\Themes\Contracts\ApplicationWrapperContract;
use Vsw\Themes\Misc\ViewExpressionTrait;
use ThemesFunc;
...
class WidgetGroup
{
    ...
    public function display()
    {
        ...
        foreach ($this->widgets as $position => $widgets) {
            foreach ($widgets as $widget) {
                $datawg = [
                    'title'=>$widget['arguments'][1]['title'],
                    'description'=>$widget['arguments'][1]['description'],
                    'coverwidget'=>$widget['arguments'][1]['coverwidget'],
                    'position'=>strtolower($widget['arguments'][1]['position']),
                    'custom_id'=>$widget['arguments'][1]['custom_id'],
                    'custom_class'=>$widget['arguments'][1]['custom_class'],
                ];
                $output .= ThemesFunc::SessionToolWidget($widgets, $this -> performWrap($this -> displayWidget($widget), $index, $count,$datawg));
                $index++;
                if ($index !== $count) {
    ...
    {
        $factory = $this->app->make($widget['type'] === 'sync' ? 'vswwidget.widget' : 'vswwidget.async-widget');
     ...
     * @return string
     */
    protected function performWrap($content, $index, $total,$datawg)
    {
        ...
        return $callback($content, $index, $total,$datawg);
    }
}
