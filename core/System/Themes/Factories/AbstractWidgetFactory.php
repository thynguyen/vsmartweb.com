...
namespace Vsw\Themes\Factories;

use Vsw\Themes\AbstractWidget;
use Vsw\Themes\Contracts\ApplicationWrapperContract;
use Vsw\Themes\Misc\EncryptException;
use Vsw\Themes\Misc\InvalidWidgetClassException;
use Vsw\Themes\Misc\ViewExpressionTrait;
use Vsw\Themes\WidgetId;
...
abstract class AbstractWidgetFactory
{
    ...
    protected function instantiateWidget(array $params = [])
    {
        ...
        if (preg_match('#^(.*?)::(.*?)$#', $str, $m)) {
            $rootNamespace = $this->app->get('vswwidget.widget-namespaces')->getNamespace($m[1]);
            $str = $m[2];
        }
        ...
        if (!is_subclass_of($widgetClass, 'Vsw\Themes\AbstractWidget')) {
            throw new InvalidWidgetClassException('Class "'.$widgetClass.'" must extend "Vsw\Themes\AbstractWidget" class');
        }
