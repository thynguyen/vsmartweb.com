...
@section('breadcrumbs')
{{Breadcrumbs::render('module_members_edit',$infomem)}}
@endsection
@section('header')
...
{!!Form::open(['method' => 'POST', 'route' => ['members.web.edit'], 'enctype'=> 'multipart/form-data'])!!}
...
<div class="card">
    ...
    <div class="card-body">
        {!! Form::textarea('about',$infomem['about'], ['class' =>'form-control','id'=>'about','rows'=>5]) !!}
    </div>
</div>
<div class="card">
    <div class="card-header text-white bg-primary {{(!$infomem['password'])?'':'collapsed'}}" id="showchangepass" data-toggle="collapse" data-target="#ChangePassword" aria-expanded="true"> 
        <strong>{!!transmod('members::Password')!!}</strong>
    </div>
    <div class="collapse {{(!$infomem['password'])?'show':''}}" id="ChangePassword">
        <div class="card-body">
            @if($infomem['password'])
            <div class="form-group row">
